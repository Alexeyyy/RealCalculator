...
void LCD_FillRectangle(uint16_t x0, uint16_t y0, uint16_t width, uint16_t height , uint16_t color) {
	int i,j;
	for(i = x0; i < width; i++) {
		for(j = 0; j < height; j++) {
...
void LCD_FillRectangle2(uint16_t x0, uint16_t y0, uint16_t width, uint16_t height , uint16_t color) {
	int i,j,k,m; //x-runners
	int a,b,c,d; //y-runners
	for(i = x0 + 1, j = x0 + 1, k = width - 1, m = width - 1; i <= width / 2 && j <= width / 2 && k >= width / 2 && m >= width /2; i++, j++, k--, m--) {
		for(a = y0 + 1, b = height - 1, c = y0 + 1, d = height - 1; a <= height / 2 && b >= height / 2 && c <= height / 2 && d >= height / 2; a++, b--, c++,d--) {
			LCD_SetPoint(i,a,color);
			LCD_SetPoint(j,b,color);
			LCD_SetPoint(k,c,color);
			LCD_SetPoint(m,d,color);
		}
	}
}
/*==================================================================================================*/
...
* Attention		 : None
*******************************************************************************/
void PutChar1( uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor )
{
	uint16_t i, j;
